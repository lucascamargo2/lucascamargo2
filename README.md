# Lucas Camargo

Information Systems Student at Feevale  
Focused on Java, Web Development and Data

## About Me
- Information Systems student
- Currently learning Java and Data Structures
- Focused on building real-world projects

## Tech Stack
![My Skills](https://skillicons.dev/icons?i=java,js,html,css,git,github,vscode,wordpress,mysql,ai,ae)

## Featured Projects
- DirTools → Java CLI for directory analysis using recursion
- Portfolio Website → Personal web portfolio
- OnlyGadgets → Technical support system (in progress)

## GitHub Stats
![Lucas's GitHub stats](https://github-readme-stats.vercel.app/api?username=lucascamargo2&show_icons=true)

![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=lucascamargo2&layout=compact)

![GitHub Streak](https://streak-stats.demolab.com?user=lucascamargo2)

## Contact
- LinkedIn: [(https://www.linkedin.com/in/lucas-camargo-519156270/)]
- Email: lucascsouza22@gmail.com
